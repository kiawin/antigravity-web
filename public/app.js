  ...
  try {
    ...
    if (data.html) {
      ...
      if (data.buttonStates) {
        ...
        const isGenerating =
          data.html.includes("input-send-button-cancel-tooltip") ||
  ...
  if (target) {
    ...
    if (action) {
      ...
      const allButtons = Array.from(
        UI.chatContainer.querySelectorAll(selector),
      ).filter((el) => {
...
async function sendMessage() {
  ...
  const _previousValue = UI.messageInput.value;
...
async function showWorkspaceSelector() {
  ...
  const options = data.workspaces.map((w) => {
    ...
    const checkMark = isCurrent
      ? '<svg viewBox="0 0 24 24" style="width:16px;height:16px;margin-left:auto;color:var(--success)"><path d="M20 6L9 17l-5-5" fill="none" stroke="currentColor" stroke-width="2"/></svg>'
      : "";
    ...
    openModal("Switch Conversation", options, async (conv) => {
      try {
        ...
        if (UI.conversationTitleText) {
          ...
          if (UI.conversationHeader)
            UI.conversationHeader.style.display = "flex";
        }
